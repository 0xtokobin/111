<template><div><h2 id="composition-api-组合式-api" tabindex="-1"><a class="header-anchor" href="#composition-api-组合式-api" aria-hidden="true">#</a> Composition API（组合式 API）</h2>
<h3 id="setup" tabindex="-1"><a class="header-anchor" href="#setup" aria-hidden="true">#</a> setup</h3>
<p>Vue3.0 中一个新的配置项，值为一个函数。</p>
<p>setup 是所有 Composition API（组合 API）“表演的舞台”</p>
<p>组件中所用到的：数据、方法等等，均要配置在 setup 中。</p>
<p>setup 函数的两种返回值：</p>
<ol>
...
<ol>
<li>尽量不要与 Vue2.x 配置混用</li>
</ol>
<p>Vue2.x 配置（data、methods、computed）可以访问到 setup 中的属性或方法，但在 setup 中不能访问到 Vue2.x 的配置。如果两者配置重名，以 setup 优先。</p>
<ol start="2">
<li>setup 不能是一个 async 函数，因为返回值不再是 return 的对象, 而是 promise, 模板看不到 return 对象中的属性。（后期也可以返回一个 Promise 实例，但需要 Suspense 和异步组件的配合）</li>
</ol>
<h3 id="setup-执行时机" tabindex="-1"><a class="header-anchor" href="#setup-执行时机" aria-hidden="true">#</a> setup 执行时机</h3>
<p>在 beforeCreate 之前执行一次，this 是 undefined</p>
<p>setup 的参数：</p>
<ul>
<li>props：值为对象，包含组件外部传递进来，且组件内部声明接收了的属性</li>
<li>context：上下文对象，包含属性
<ul>
<li>attrs：值为对象，包含组件外部传递过来，但没有在 props 配置中声明的属性，相当于<code v-pre>this.$attrs</code></li>
<li>slots：收到的插槽内容，相当于<code v-pre>this.$slots</code></li>
<li>emit：分发自定义事件的函数，相当于<code v-pre>this.$emit</code></li>
</ul>
</li>
</ul>
<h3 id="ref-函数" tabindex="-1"><a class="header-anchor" href="#ref-函数" aria-hidden="true">#</a> ref 函数</h3>
<p>作用：定义一个响应式数据</p>
<p>语法：<code v-pre>const xx = ref(initValue)</code></p>
<ul>
<li>创建一个包含响应式数据的引用对象（reference 对象，简称 ref 对象）</li>
<li>在 js 中操作数据：<code v-pre>xx.value = xxxxx</code></li>
<li>模板中读取数据：不需要<code v-pre>.value</code>，直接写入插值语句<code v-pre>&lt;div&gt;{{ xx }}&lt;/div&gt;</code></li>
<li>接收的数据可以是基本类型，也可以是对象类型（最好是基本类型）</li>
<li>基本类型的数据：响应式依然是靠<code v-pre>Object.defineProperty()</code>的 get 与 set 完成的</li>
<li>对象类型的数据：内部借助 Vue3 的 reactive 函数</li>
</ul>
<h3 id="reactive-函数" tabindex="-1"><a class="header-anchor" href="#reactive-函数" aria-hidden="true">#</a> reactive 函数</h3>
<p>作用：定义一个对象类型的响应式数据（基本类型只使用 ref 函数）</p>
<p>语法：<code v-pre>const obj = reactive({ name: '123' })</code></p>
<ul>
<li>接收一个对象（或数组》，返回一个代理对象（Proxy 的实例对象，简称 proxy 对象）</li>
<li>reactive 定义的响应式数据是深层次的</li>
<li>内部基于 ES6 的 Proxy 实现，通过代理对象操作源对象内部数据进行操作</li>
</ul>
<h3 id="计算与监视属性" tabindex="-1"><a class="header-anchor" href="#计算与监视属性" aria-hidden="true">#</a> 计算与监视属性</h3>
<h4 id="computed" tabindex="-1"><a class="header-anchor" href="#computed" aria-hidden="true">#</a> computed</h4>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="watch" tabindex="-1"><a class="header-anchor" href="#watch" aria-hidden="true">#</a> watch</h4>
...
<p>watch 既要指明监视的属性，也要指明监视的回调。</p>
<p>watchEffect 不用指明监视哪个属性，监视的回调中用到哪个属性，就监视哪个属性。</p>
<p>watchEffect 有点像 computed：</p>
<ul>
<li>但 computed 注重计算出来的值（回调函数的返回值），所以必须要谢返回值</li>
<li>而 watchEffect 更注重过程（回调函数的函数体），所以不用写返回值</li>
</ul>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="生命周期" tabindex="-1"><a class="header-anchor" href="#生命周期" aria-hidden="true">#</a> 生命周期</h3>
<p>以下是 vue3 的生命周期图示</p>
<figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/Vue3/lifecycle.16e4c08e.png" alt="Vue3生命周期" tabindex="0" loading="lazy"><figcaption>Vue3生命周期</figcaption></figure>
<p>Vue3 中可以继续使用 Vue2 的生命周期钩子，但有两个更名：</p>
<p><code v-pre>beforeDestroy</code>改名为 <code v-pre>beforeUnmount</code></p>
<p><code v-pre>destroyed</code>改名为 <code v-pre>unmounted</code></p>
...
<h3 id="自定义-hook-函数" tabindex="-1"><a class="header-anchor" href="#自定义-hook-函数" aria-hidden="true">#</a> 自定义 hook 函数</h3>
<p>什么是 hook？—— 本质是一个函数，把 setup 函数中使用的 Composition API 进行了封装。</p>
<p>类似于 vue2.x 中的 mixin。</p>
<p>自定义 hook 的优势: 复用代码, 让 setup 中的逻辑更清楚易懂。</p>
<p>以下是一个暴露响应式宽高数据和获取窗口宽高方法的 hooks 函数</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="组合式-api-的优势" tabindex="-1"><a class="header-anchor" href="#组合式-api-的优势" aria-hidden="true">#</a> 组合式 API 的优势</h3>
<h4 id="options-api-存在的问题" tabindex="-1"><a class="header-anchor" href="#options-api-存在的问题" aria-hidden="true">#</a> Options API 存在的问题</h4>
<p>使用传统 OptionsAPI 中，新增或者修改一个需求，就需要分别在 data，methods，computed 里修改。</p>
<figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/Vue3/optionsAPI.image" alt="OptionsAPI" tabindex="0" loading="lazy"><figcaption>OptionsAPI</figcaption></figure>
<h4 id="composition-api-的优势" tabindex="-1"><a class="header-anchor" href="#composition-api-的优势" aria-hidden="true">#</a> Composition API 的优势</h4>
<p>我们可以更加优雅的组织我们的代码，函数。让相关功能的代码更加有序的组织在一起。</p>
<figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/Vue3/compositionAPI.image" alt="CompositionAPI" tabindex="0" loading="lazy"><figcaption>CompositionAPI</figcaption></figure>
<h2 id="响应式原理" tabindex="-1"><a class="header-anchor" href="#响应式原理" aria-hidden="true">#</a> 响应式原理</h2>
<h3 id="vue2-响应式" tabindex="-1"><a class="header-anchor" href="#vue2-响应式" aria-hidden="true">#</a> Vue2 响应式</h3>
<p>实现原理：</p>
<ul>
<li>对象类型：通过<code v-pre>Object.defineProperty()</code>对属性的读取、修改进行拦截（数据劫持）</li>
<li>数组类型：通过重写更新数组的一系列方法实现拦截（对数组的变更方法进行了包裹）</li>
<li>存在问题：新增属性、删除属性，界面不更新；直接通过下标修改数组，界面不更新</li>
</ul>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="vue3-响应式" tabindex="-1"><a class="header-anchor" href="#vue3-响应式" aria-hidden="true">#</a> Vue3 响应式</h3>
<p>实现原理：</p>
<ul>
<li>通过 Proxy 代理：拦截对象中任意属性的变化，包括：属性值的读写、属性的添加、属性的删除等。</li>
<li>通过 Reflect 反射：对源对象的属性进行操作。</li>
<li>MDN 文档中描述的 Proxy 和 Reflect：</li>
<li>Proxy：<a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Proxy" target="_blank" rel="noopener noreferrer">https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Proxy<ExternalLinkIcon/></a></li>
<li>Reflect：<a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Reflect" target="_blank" rel="noopener noreferrer">https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Reflect<ExternalLinkIcon/></a></li>
</ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="ref-与-reactive-对比" tabindex="-1"><a class="header-anchor" href="#ref-与-reactive-对比" aria-hidden="true">#</a> ref 与 reactive 对比</h3>
<ul>
<li>从定义数据角度对比：</li>
</ul>
<p>ref 用来定义基本类型数据</p>
<p>reactive 定义复杂类型数据</p>
<p>注：ref 也可定义复杂类型，它内部实际上自动通过 reactive 转为代理对象</p>
<ul>
<li>从原理角度对比：</li>
</ul>
<p>ref 通过<code v-pre>Object.defineProperty()</code>实现响应式（数据劫持）</p>
<p>reactive 通过使用 Proxy 实现响应式（数据代理），并通过 Reflect 操作原对象内部数据</p>
<ul>
<li>从使用角度对比：</li>
</ul>
<p>ref 定义的数据：操作数据需要<code v-pre>.value</code>去读写数据，模板读取时直接读取，不需要<code v-pre>.value</code></p>
<p>reactive 定义的数据：操作数据与模板读取数据均不需要<code v-pre>.value</code></p>
</div></template>
