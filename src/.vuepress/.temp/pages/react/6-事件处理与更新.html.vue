<template><div><h2 id="事件处理" tabindex="-1"><a class="header-anchor" href="#事件处理" aria-hidden="true">#</a> 事件处理</h2>
<ol>
<li>通过 onClick、onMousemove、onMouseenter 等属性指定事件处理函数（注意大小写）</li>
<li>React 使用的是自定义事件，而不是原生 DOM 事件，为了更好的兼容性</li>
<li>React 中的事件是通过事件委托方式处理的（委托给组件外层的元素）</li>
<li>通过<code v-pre>event.target</code>可以得到发生事件的 DOM 元素对象</li>
</ol>
<h2 id="高阶函数" tabindex="-1"><a class="header-anchor" href="#高阶函数" aria-hidden="true">#</a> 高阶函数</h2>
<p>如果一个函数符合下面 2 个规范中的任何一个，那该函数就是高阶函数</p>
<ol>
<li>若 A 函数，接收的参数是一个函数，那么 A 就可以称之为高阶函数</li>
<li>若 A 函数，调用的返回值依然是一个函数，那么 A 就可以称之为高阶函数</li>
</ol>
<p>常见高阶函数：Promise、定时器、数组高阶方法（map、filter 等）</p>
<p>函数柯里化：通过函数调用继续返回函数的方式，实现多次接收参数，最后统一处理的函数编码形式</p>
<p><strong>注意：</strong></p>
<ol>
<li>父子通信的函数调用需要使用柯里化维持 this 对象</li>
<li>JSX 循环时需要使用高阶函数 map 返回 JSX 对象</li>
</ol>
<h2 id="jsx-循环" tabindex="-1"><a class="header-anchor" href="#jsx-循环" aria-hidden="true">#</a> JSX 循环</h2>
<p>以下是 state 中的数组元素循环生成 JSX 的写法</p>
...
随后 React 进行【新虚拟 DOM】与【旧虚拟 DOM】的 diff 比较，比较规则如下：</p>
...
<figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/React/image-20210915190521544.png" alt="react-diff更新" tabindex="0" loading="lazy"><figcaption>react-diff更新</figcaption></figure>
<h2 id="组件树更新原理" tabindex="-1"><a class="header-anchor" href="#组件树更新原理" aria-hidden="true">#</a> 组件树更新原理</h2>
<p>React 中全部的组件树会转换为 Fiber 节点，这些 Fiber 节点连起来形成 Fiber 树（又称 vdom 树），Fiber 树中判断节点的复用条件有：</p>
<ol>
<li>Fiber 节点的 type 属性是否发生变化；</li>
<li>Fiber 节点的 props 属性是否发生变化；</li>
<li>Fiber 节点的 state 是否发生变化；</li>
<li>Fiber 节点的 context 是否发生变化；</li>
</ol>
<p>如果以上的条件都为否，那么就可以判断这个节点没有发生变化，不需要重新渲染</p>
<p>但是！！！因为 React 并没有采用 Vue 那样的方式，以组件作为更新单位，中等粒度地找出哪些节点发生了更新，所以 React 每次触发更新都会从头开始生成一个新的 Fiber 树，然后与上一次更新之后生成的旧 Fiber 树对比，判断上述几个条件，找出发生了变化的 Fiber 节点，再将其更新到页面上</p>
<p>这样逻辑上虽然可行，但是每次都从头开始对比整棵树，难免会做很多冗余比较。因此 React 触发更新之后，实际会从当前触发更新的节点开始向上对其所有的父节点打上有子节点需要更新的标记，这时会新增第 5 项判断：<strong>子节点是否存在变化</strong>，满足前 4 条件后，如果连同满足额外的第 5 个条件，那么当前节点及其所有子节点都可以跳过对比直接复用！</p>
<p>所以完整的五项更新 or 复用判断条件是：</p>
<ol>
<li>Fiber 节点的 type 属性是否发生变化；</li>
<li>Fiber 节点的 props 属性是否发生变化；</li>
<li>Fiber 节点的 state 是否发生变化；</li>
<li>Fiber 节点的 context 是否发生变化；</li>
<li>Fiber 节点是否存在子节点更新标记</li>
</ol>
<p>React 组件树中有组件触发更新后，从顶层父节点开始递归遍历各个子节点</p>
<ul>
<li>前 4 项条件有一项发生变化，当前组件及其下属子组件全部 rerender</li>
<li>前 4 项条件有一项发生变化，第 5 项存在更新标记，父组件自身不会 rerender，但是子组件继续进入对比判断</li>
<li>上述 5 项条件均无变化的情况，父组件及下属子组件均不触发 rerender</li>
</ul>
</div></template>
