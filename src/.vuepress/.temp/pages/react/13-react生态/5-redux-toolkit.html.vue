<template><div><p><code v-pre>redux-toolkit</code>是目前 redux 官方推荐的编写 redux 逻辑的方法，针对 redux 本身创建 store 繁琐、样板代码过多、依赖外部库等问题进行了优化，调用 API 更加简单快捷</p>
<p>redux 最新版本也已经标记 createStore 方法为弃用，推荐使用 redux-toolkit</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">yarn</span> <span class="token function">add</span> @reduxjs/toolkit react-redux
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h2 id="入口文件" tabindex="-1"><a class="header-anchor" href="#入口文件" aria-hidden="true">#</a> 入口文件</h2>
<p>没有变化</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="store-js" tabindex="-1"><a class="header-anchor" href="#store-js" aria-hidden="true">#</a> store.js</h2>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">/* app/store.ts */</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>configureStore 集成了 redux 之前的各种扩展，以对象形式传入，并默认支持<code v-pre>redux-DevTools</code></p>
<p>参数选项表</p>
<p>| 参数 key 值 | 说明 |<br>
| reducer | 创建 reducer，传递给 combineReducers 使用 |<br>
| middleware | 中间件，传递给 applyMiddleware 使用 |<br>
| devTools | 扩展工具，默认为 true |<br>
| preloadedState | 初始 state 值，传递给 createStore |<br>
| enhancers | 增强 store，传递给 createStore |</p>
<h2 id="actions-reducers" tabindex="-1"><a class="header-anchor" href="#actions-reducers" aria-hidden="true">#</a> actions&amp;reducers</h2>
<p><code v-pre>redux-toolkit</code>加入了 slice，对 action 和 reducer 实现了整合</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> createSlice <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">"@reduxjs/toolkit"</span><span class="token punctuation">;</span>
...
<li>reducers 参数既定义了 reducers，也创建了关联的 actions，最终返回的是一个包含了 actions 和 reducers 的对象</li>
<li>这里直接对 state 进行了修改，因为引用了 immer 的库，底层也是响应式的，总是会返回一个安全不可变的更新值，简化了 reducer 的写法。注意该方式只能在 createSlice 和 createReducer 中编写</li>
<li>异步方法最直接的写法就是采用<code v-pre>redux-thunk</code>的方式书写，因为内部已经默认集成了该异步中间件</li>
</ul>
<h2 id="视图组件调用" tabindex="-1"><a class="header-anchor" href="#视图组件调用" aria-hidden="true">#</a> 视图组件调用</h2>
<div class="language-jsx line-numbers-mode" data-ext="jsx"><pre v-pre class="language-jsx"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> useSelector<span class="token punctuation">,</span> useDispatch <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">"react-redux"</span><span class="token punctuation">;</span>
...
<ul>
<li>任意组件都能从<code v-pre>redux store</code>中读取任意数据</li>
<li>任意组件都能通过<code v-pre>dispatch actions</code>引发状态更新（state updates）</li>
</ul>
<h2 id="进阶1-副作用处理" tabindex="-1"><a class="header-anchor" href="#进阶1-副作用处理" aria-hidden="true">#</a> <code v-pre>进阶1：副作用处理</code></h2>
<p>createAsyncThunk 支持 thunk，这也是 redux 推荐的方式</p>
<p>下面是推荐写法</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">const</span> incrementAsync <span class="token operator">=</span> <span class="token function">createAsyncThunk</span><span class="token punctuation">(</span>
...
<span class="token keyword">export</span> <span class="token keyword">const</span> counterSlice <span class="token operator">=</span> <span class="token function">createSlice</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="进阶2-使用-selector-缓存" tabindex="-1"><a class="header-anchor" href="#进阶2-使用-selector-缓存" aria-hidden="true">#</a> <code v-pre>进阶2：使用 selector 缓存</code></h2>
<p>当 useSelector 方法涉及到复杂逻辑运算时，且返回一个对象的时候，每次运行都返回了一个新的引用值，会使组件重新渲染，即使返回的数据内容并没有改变，如下带有过滤的 todoList 所示</p>
<p>其原因是复杂数据类型栈地址的变化引发更新机制</p>
...
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">const</span> <span class="token function-variable function">selectTodos</span> <span class="token operator">=</span> <span class="token punctuation">(</span>state<span class="token operator">:</span> RootState<span class="token punctuation">)</span> <span class="token operator">=></span> state<span class="token punctuation">.</span>todo<span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="进阶3-范式化state" tabindex="-1"><a class="header-anchor" href="#进阶3-范式化state" aria-hidden="true">#</a> <code v-pre>进阶3：范式化state</code></h2>
<p>范式化 state 指的是：</p>
<ul>
<li>state 中每个特定数据只有一个副本，不存在重复</li>
<li>已范式化的数据保存在表中，项目 ID 是键，项本身是值</li>
<li>也可能有一个特定想用于保存所有 ID 的数组</li>
</ul>
<p>以上的标准决定了 state 的结构形式为：</p>
...
<p><code v-pre>redux-toolkit</code>仍是以 redux 结构为原型开发的状态管理库，它对 redux 和 react-redux 原本代码冗余、创建复杂的步骤进行了优化，并加入了多种异步解决方案、范式化修改、数据缓存等功能，并提供了 hooks 函数供任意组件使用，是 redux 目前最推荐使用的库</p>
</div></template>
