<template><div><p>本章记录基于 Flux 思想在内的多数常用状态管理库，不局限于 Redux</p>
<h2 id="flux架构" tabindex="-1"><a class="header-anchor" href="#flux架构" aria-hidden="true">#</a> <code v-pre>Flux</code>架构</h2>
<p>Flux 是一种架构思想，解决软件的结构问题，它跟 MVC 架构是同一类东西，但是更加简单和清晰</p>
<p>Facebook Flux 的应用架构利用<strong>单向数据流</strong>组合 React 中的视图组件，具体流程是：</p>
<ol>
<li>用户访问 View</li>
<li>View 发出用户的 Action</li>
<li>Dispatcher 收到 Action，要求 Store 进行相应更新</li>
<li>Store 更新后，发出一个'change'事件</li>
<li>View 收到'change'事件后，更新页面</li>
</ol>
<h2 id="redux" tabindex="-1"><a class="header-anchor" href="#redux" aria-hidden="true">#</a> <code v-pre>Redux</code></h2>
<p>Redux 作为应用全局状态管理机而存在，用一个单独的常量状态树（state 对象）保存这一整个应用状态，并且对象不能直接被改变，当数据发生变化，一个新的对象就会被创建（使用 actions 和 reducers），再进行数据跟踪，实现时光旅行</p>
<h3 id="redux使用三原则" tabindex="-1"><a class="header-anchor" href="#redux使用三原则" aria-hidden="true">#</a> <code v-pre>Redux</code>使用三原则</h3>
<ol>
<li>state 以单一对象存储在 store 对象中</li>
<li>state 只读（每次都返回一个新对象）</li>
<li>使用纯函数 reducer 执行 state 更新</li>
</ol>
<h3 id="redux工作流" tabindex="-1"><a class="header-anchor" href="#redux工作流" aria-hidden="true">#</a> <code v-pre>Redux</code>工作流</h3>
<figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/React/redux-workflow.jfif" alt="Redux工作流" tabindex="0" loading="lazy"><figcaption>Redux工作流</figcaption></figure>
<h3 id="store" tabindex="-1"><a class="header-anchor" href="#store" aria-hidden="true">#</a> Store</h3>
<p>redux 中只有一个 Store，整个应用需要管理的数据都在这个 Store 里面，这个 Store 我们不能直接去改变，只能通过返回新的 Store 去更改它</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="action" tabindex="-1"><a class="header-anchor" href="#action" aria-hidden="true">#</a> Action</h3>
<p>这个 action 指的是视图层发起的一个操作，告诉 Store 我们需要改变。比如用户点击了按钮，我们就要去请求列表，列表的数据就会变更。每个 action 必须有一个 type 属性，这表示 action 的名称，然后还可以有一个 payload 属性，这个属性可以带一些参数，用作 Store 变更</p>
<p>Action 是消息的载体，只能被别人操作自己不进行操作</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> action <span class="token operator">=</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="reducer" tabindex="-1"><a class="header-anchor" href="#reducer" aria-hidden="true">#</a> Reducer</h3>
<p><code v-pre>store.dispatch()</code>是 View 发出 Action 的唯一方法，发起 Action 之后会到达 Reducer</p>
<p>Reducer 接收两个参数：当前的 state 和接收到的 action，经过计算，返回一个新的 state</p>
<p>reducer 是一个纯函数，即对于相同的输入，只会有相同的输出，不影响外部的值</p>
<h3 id="示例代码" tabindex="-1"><a class="header-anchor" href="#示例代码" aria-hidden="true">#</a> 示例代码</h3>
<p>store.js</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">//引入creacteStore,专门用于创建redux中最核心的store对象</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>reducers.js</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">//接收两个参数 之前的状态preState,和动作对象action</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="reducer命名空间" tabindex="-1"><a class="header-anchor" href="#reducer命名空间" aria-hidden="true">#</a> <code v-pre>Reducer</code>命名空间</h3>
<p>不同的 actions 所处理的属性之间没有联系的，可以把 Reducer 函数拆分，不同函数分管不同属性最终合并 Store</p>
<p>先使用 combineReducers 合并 reducers，在 createStore</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>访问：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token punctuation">(</span><span class="token parameter">state</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">(</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="react-redux" tabindex="-1"><a class="header-anchor" href="#react-redux" aria-hidden="true">#</a> react-redux</h2>
<p><code v-pre>react-redux</code>简化了 redux 的书写流程，使 redux 能更简洁地嵌入到 React 组件中，比如无需组件添加与取消订阅，属性自动获取等</p>
<h3 id="容器组件与-ui-组件" tabindex="-1"><a class="header-anchor" href="#容器组件与-ui-组件" aria-hidden="true">#</a> 容器组件与 UI 组件</h3>
<p>UI 组件：</p>
<ul>
<li>只负责 UI 呈现，不带有任何业务逻辑</li>
<li>没有状态，即不使用 state 状态管理</li>
<li>所有数据都由参数 props 提供</li>
...
<h3 id="react-redux流程" tabindex="-1"><a class="header-anchor" href="#react-redux流程" aria-hidden="true">#</a> <code v-pre>react-redux</code>流程</h3>
<figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/React/react-redux.png" alt="react-redux流程" tabindex="0" loading="lazy"><figcaption>react-redux流程</figcaption></figure>
<p>简化的流程有：</p>
<ol>
<li>store -&gt; reducers -&gt; combineReducers：reducer 合并</li>
<li>store -&gt; mapStateToProps：状态转 props 属性</li>
<li>actions -&gt; mapDispatchToProps：方法转属性</li>
<li>最终容器组件 = connect(mapStateToProps, mapDispatchToProps：方法转属性)(视图组件)：属性与方法注入</li>
<li>提供者 Provider：提供全局 store</li>
</ol>
<p>总的来说，<code v-pre>react-redux</code>是为了简化 redux 的操作步骤，对视图组件执行 props 属性的按需注入，通过属性直接操作 state 与 action，进一步分离了 redux 与 view 之间的联系，view 组建中不会调用 redux 中的任何 api</p>
<p>从代码风格上来看，<code v-pre>react-redux</code>跟<code v-pre>vueX</code>风格类似</p>
<p>示例代码详见<a href="#%E4%BD%BF%E7%94%A8%E6%A1%88%E4%BE%8B">redux-saga 代码示例</a>引用了<code v-pre>react-redux</code>的案例</p>
</div></template>
